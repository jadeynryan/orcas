...
cwr_untidy <- get_all_encounters(years = 2017:2023)
...
cwr_tidy <- cwr_untidy |>
  ...
  tidyr::unite(end_latitude,
               tidyr::any_of(c("end_latitude",
                               "e_latitude")),
               sep = "",
               na.rm = TRUE) |>
  tidyr::unite(end_longitude,
               tidyr::any_of(c("end_longitude",
                               "e_longitude")),
               sep = "",
               na.rm = TRUE) |>
  tidyr::unite(vessel,
  ...
  dplyr::mutate(
    ...
    date = dplyr::case_when(
      date == lubridate::date("0218-09-30") ~ lubridate::date("2018-09-30"),
      date == lubridate::date("2014-08-08") ~ lubridate::date("2018-07-24"),
  ...
  dplyr::mutate(
    ...
    duration = lubridate::as.duration(lubridate::interval(begin_time, end_time)),
    .after = "end_time",
  ...
  dplyr::mutate(dplyr::across(dplyr::ends_with("longitude"),
                              \(x) abs(x) * -1)) |>
  # Change coordinates to NaN if they are out of bounds
  dplyr::mutate(
    begin_latitude = dplyr::case_when(begin_latitude < 45 ~ NaN,
                                      TRUE ~ begin_latitude),
    end_latitude = dplyr::case_when(end_latitude < 45 ~ NaN,
                                    TRUE ~ end_latitude),
    begin_longitude = dplyr::case_when(begin_longitude > -120 ~ NaN,
                                       TRUE ~ begin_longitude),
    end_longitude = dplyr::case_when(end_longitude > -120 ~ NaN,
                                     TRUE ~ end_longitude)
  ) |>
